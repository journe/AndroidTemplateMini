...
fun ViewPager2.setAnimation(scale: Float = 0.1f, opacity: Float = 0.5f) {
	offscreenPageLimit = 2
	setPageTransformer { page, position ->
		page.translationX = (-SizeUtils.dp2px(64f)) * position
//		page.translationY = 20 * position
		val scaleFactor = 1 - abs(position) * scale
		page.scaleX = scaleFactor
		page.scaleY = scaleFactor

		page.alpha = 1 - abs(position) * opacity
	}
