package com.template.common.navigation
...
class FragmentNavigatorHideShow(
    ...
    private fun navigate(
        ...
        if (frag != null) {
            fragmentTransaction.setMaxLifecycle(frag, Lifecycle.State.STARTED)
        ...
        if (frag != null) {
            fragmentTransaction.setMaxLifecycle(frag, Lifecycle.State.RESUMED)
            fragmentTransaction.show(frag)
        } else {
            frag = mFragmentManager.fragmentFactory.instantiate(mContext.classLoader, className)
            frag.arguments = args
            fragmentTransaction.add(mContainerId, frag, tag)
        }
        //endregion
        ...
        val isAdded = when {
            ...
            isSingleTopReplacement -> {
                ...
                if (backStack.size > 1) {
                    ...
                    fragmentTransaction.addToBackStack(entry.id)
                }
                false
            }
            else -> {
