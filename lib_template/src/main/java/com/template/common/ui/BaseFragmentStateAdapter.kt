...
abstract class BaseFragmentStateAdapter<T> : FragmentStateAdapter {
    ...
    fun replaceData(position: Int) {
        if (position < mData.size && position < mFragmentIdMap.size) {
            val oldData = copyData()

//            mFragmentIdMap   mutableMapOf<T, Long>()
            // mealBean : fragmentID
            //  mData :mealBeanlist
            //
            mFragmentIdMap[mData[position]] = Random.nextLong() - Random.nextInt()

            val newFragment = createFragment(mData[position], position)
            diffNotifyDataSetChanged(oldData, mData)
        }
    }
     ...
     */
    open fun diffNotifyDataSetChanged(oldData: MutableList<T>, newData: MutableList<T>) {
